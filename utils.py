...
class MetricLogger(object):
    ...
    def log_every(self, iterable, print_freq, header=None):
        ...
        if len(iterable) > 0:
            ...
            print('{} Total time: {} (empty iterable, no items processed)'.format(
...
def init_distributed_mode(args):
    ...
    if args.dist_on_itp:
        ...
        # Set default values for non-distributed mode
        if not hasattr(args, 'gpu') or args.gpu is None:
            # If device is cuda, set gpu to 0, otherwise None
            if hasattr(args, 'device') and 'cuda' in str(args.device):
                args.gpu = 0
            else:
                args.gpu = None
        args.rank = 0
...
def auto_load_model(args, model, model_without_ddp, optimizer, loss_scaler, model_ema=None):
    ...
    if args.resume:
        ...
        try:
            ...
            if len(missing_keys) > 0:
                print(f"Warning: {len(missing_keys)} keys not found in checkpoint (will use random initialization):")
                for key in missing_keys[:10]:  # Print first 10 missing keys
                    print(f"  - {key}")
            ...
            if len(unexpected_keys) > 0:
                print(f"Info: {len(unexpected_keys)} unexpected keys in checkpoint (will be ignored):")
                for key in unexpected_keys[:10]:  # Print first 10 unexpected keys
                    print(f"  - {key}")
                if len(unexpected_keys) > 10:
                    print(f"  ... and {len(unexpected_keys) - 10} more keys")
        except Exception as e:
            # Fallback to strict loading if non-strict fails
